...
const BLENDER_OUT = `blender${getExt(BLENDER_DOWNLOAD_URL)}`;
...
async function runExtract(fileName: string, destination: string): Promise<void> {
  if (fileName.endsWith('.zip')) {
    await exec('7z', ['x', fileName, `-o${destination}`, '-y']);
  } else if (fileName.endsWith('.tar.xz')) {
    await exec('tar', ['-xf', fileName, '-C', destination]);
...
async function downloadBlender(): Promise<void> {
  ...
  const blenderPath = path.join(BLENDER_WORKING_PATH, BLENDER_OUT);
...
async function prepareBlender(): Promise<void> {
  const zipFile = path.join(BLENDER_WORKING_PATH, BLENDER_OUT);
